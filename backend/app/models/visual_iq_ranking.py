...
class VisualIqRanking(Base):
    __tablename__ = "visual_iq_rankings"

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    game_id: Mapped[str] = mapped_column(String(100), nullable=False, default="visual-brain-teasers", index=True)
    player_name: Mapped[str] = mapped_column(String(120), nullable=False, index=True)
    age: Mapped[int] = mapped_column(Integer, nullable=False)
    iq_score: Mapped[int] = mapped_column(Integer, nullable=False, index=True)
    ...
    speed_percent: Mapped[int] = mapped_column(Integer, nullable=False)
